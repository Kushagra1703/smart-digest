...
import { ArrowRight, Sparkles, Zap, Shield, Layers, History } from "lucide-react";
        ...
        <main className="min-h-[calc(100vh-65px)] bg-background text-foreground transition-colors duration-500 overflow-hidden">
                ...
                <div className="max-w-5xl mx-auto text-center relative z-10">
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
                        ...
                        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                            ...
                            <SignedIn>
                                {/* Analyse Now button redirects to the dedicated analysis page */}
                                <Link href="/dashboard" className="w-full sm:w-auto px-10 py-5 bg-primary text-white text-lg font-black rounded-2xl shadow-2xl shadow-primary/30 hover:scale-105 transition-all flex items-center justify-center gap-2 group text-center">
                                    Analyse Now
                                    <Zap className="group-hover:scale-110 transition-transform" size={20} />
                                </Link>
                                
                                {/* View History button points to your card-based history grid */}
                                <Link href="/history" className="w-full sm:w-auto px-10 py-5 bg-card border border-border text-lg font-bold rounded-2xl hover:bg-slate-100 dark:hover:bg-zinc-800 transition-all flex items-center justify-center gap-2 group">
            ...
            <section className="py-20 px-6 bg-card/30 border-t border-border">
                <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
                    <div className="p-8 space-y-4 bg-background rounded-3xl border border-border shadow-sm hover:shadow-md transition-shadow">
            ...
            <footer className="py-10 text-center text-muted-foreground text-sm font-medium border-t border-border/50">
                <p>© 2026 SmartDigest AI • Developed by Kushagra</p>
            </footer>
        </main>
    );
}